...
async function load() {
  ...
  caseParameters = new CaseParameters(
    agency,
    document.getElementById("searchParameters")
  );
  ...
  Promise.all(mainPromises).then(function () {
    ...
    if (utility.getQSValue("e") == "y" && utility.getQSValue("k")) {
      ...
      if (sanctionID) {
        ...
        if (
          utility.getQSValue("e") == "y" &&
          utility.getQSValue("ed").toLowerCase() == "recent"
        ) {
    ...
    .then(function (logID) {
      ...
      $.when(pSave)
        .done(function (data) {
          ...
          caseParameters
            .reloadSavedSearches(
              saveOptions.SavedSearchID,
              saveOptions.SearchName
            )
            .done(function () {
              $("#sidebar-left-main").slideToggle(500);
              $("#sidebar-left-save-search").slideToggle(500);
        ...
        .fail(function (err) {
          kendo.alert(
            "An error occurred while saving the search parameters.<br/>" + err
          );
        })
        .always(function () {
          kendo.ui.progress($("BODY"), false);
